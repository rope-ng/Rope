...
class VideoManager():  
    def __init__(self, models ):
        self.models = models
        self.device = models.device        # Model related
...
#'   1.25 M-A:  0.000 fd=   0 aq=   12KB vq=    0KB sq=    0B f=0/0' 
    ...
    def swap_video(self, target_image, frame_number, use_markers):
        ...
        img = torch.from_numpy(target_image.astype('uint8')).to(self.device) #HxWxc
    ...
    def swap_core(self, img, kps, s_e, parameters, control): # img = RGB
        ...
        latent = torch.from_numpy(self.models.calc_swapper_latent(s_e)).float().to(self.device)
        ...
        if parameters['ColorSwitch']:
            ...
            del_color = torch.tensor([parameters['ColorRedSlider'], parameters['ColorGreenSlider'], parameters['ColorBlueSlider']], device=self.device)
            swap += del_color
            swap = torch.clamp(swap, min=0., max=255.)
            swap = swap.permute(2, 0, 1).type(torch.uint8)        

        # Create border mask
        border_mask = torch.ones((128, 128), dtype=torch.float32, device=self.device)
        ...
        # Create image mask
        swap_mask = torch.ones((128, 128), dtype=torch.float32, device=self.device)
    ...
    def apply_occlusion(self, img, amount):        
        ...
        if amount >0:                   
            ...
            outpred = torch.unsqueeze(outpred, 0)
            outpred = torch.unsqueeze(outpred, 0)
            outpred = F.conv2d(outpred, kernel, padding=1)
            outpred = torch.squeeze(outpred)
            outpred = torch.squeeze(outpred)
    ...
    def apply_face_parser(self, img, FaceAmount, MouthAmount):
        ...
        outpred = torch.ones((512,512), dtype=torch.float32, device=self.device).contiguous()        
        ...
        if FaceParserAmount != 0:
            ...
            test = torch.tensor([ 0, 14, 15, 16, 17, 18], device=self.device)
